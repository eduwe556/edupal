        ...
        if data['requestSuccessful']:
            ...
            if accounts and isinstance(accounts, list) and len(accounts) > 0:
                ...
                if first_account.get('accountNumber') and first_account.get('bankName'):
                    return {'accountNumber': first_account['accountNumber'], 'bankName': first_account['bankName'], 'accountName': body.get('accountName'), 'accounts': accounts}
    return None
...
def create_virtual_account(email, amount, reference, plan_name):
    ...
    data = {"accountReference": reference, "accountName": f"EduPal {plan_name}", "currencyCode": "NGN", "contractCode": MONNIFY_CONTRACT_CODE, "customerEmail": email, "customerName": session.get('admin_name', 'User'), "incomeSplitConfig": [], "restrictPaymentSource": False, "getAllAvailableBanks": True, "amount": amount}
        ...
        if resp_data['requestSuccessful']:
            ...
            if accounts and isinstance(accounts, list) and len(accounts) > 0:
                ...
                if account_number and bank_name:
                    ...
                    conn.commit()
                    conn.close()
                    return {'accountNumber': account_number, 'bankName': bank_name, 'accountName': body.get('accountName'), 'accounts': accounts}
...
def verify_transaction(reference):
    ...
    headers = {'Authorization': f'Bearer {token}', 'Content-Type': 'application/json'}
    data = {"paymentReference": reference, "page": 0, "size": 10}
    try:
        response = requests.post(url, headers=headers, json=data)
        if response.status_code == 200:
            resp = response.json()
...
def subscribe():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            conn = get_db()
            conn.execute('INSERT INTO subscriptions (user_id, reference, plan, amount, status, account_number, account_bank) VALUES (?, ?, ?, ?, "pending", ?, ?)', (session['user_id'], reference, plan, amount, account_number, bank_name))
            conn.commit()
            conn.close()
            ...
            return redirect(url_for('subscribe'))
    return render_template('subscribe.html', basic_price=basic_price, silver_price=silver_price, gold_price=gold_price)
...
if __name__ == '__main__':
    app.run()        today = date.today().isoformat()
        return today <= sub['subscription_end']
    return False
